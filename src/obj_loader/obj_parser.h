...
typedef struct obj_material
{
	...
	char ambient_texture_filename[OBJ_FILENAME_LENGTH];
	char diffuse_texture_filename[OBJ_FILENAME_LENGTH];
	char specular_texture_filename[OBJ_FILENAME_LENGTH];
	char spec_exp_texture_filename[OBJ_FILENAME_LENGTH];
	char dissolve_texture_filename[OBJ_FILENAME_LENGTH];
	char decal_texture_filename[OBJ_FILENAME_LENGTH];
	char displacement_texture_filename[OBJ_FILENAME_LENGTH];
	char bump_texture_filename[OBJ_FILENAME_LENGTH];
	char reflect_texture_filename[OBJ_FILENAME_LENGTH];
	double amb[3];
	double diff[3];
	double spec[3];
...
int parse_obj_scene_string(obj_scene_data *data_out, char const * obj_data_string, char const * mtl_data_string);
...
int find_next(const char * string, int start, int end, char tofind)
{
	if(string[start] == '\0')
		return start;
...
const char* get_next_line(char* dest, const char * src, unsigned int dest_size)
{
	int line_start = 0;
	int line_end;
	int line_length;
	...
	line_end = find_next(src, line_start, line_start+dest_size, '\n');
	line_length = line_end - line_start;
	start_ptr = &src[line_start];
	end_ptr = &src[line_end];
...
void obj_set_material_defaults(obj_material *mtl)
{
	...
	mtl->refract_index = 1;
	mtl->illumination = 0;
	mtl->ambient_texture_filename[0] = '\0';
	mtl->diffuse_texture_filename[0] = '\0';
	mtl->specular_texture_filename[0] = '\0';
	mtl->spec_exp_texture_filename[0] = '\0';
	mtl->dissolve_texture_filename[0] = '\0';
	mtl->decal_texture_filename[0] = '\0';
	mtl->displacement_texture_filename[0] = '\0';
	mtl->bump_texture_filename[0] = '\0';
	mtl->reflect_texture_filename[0] = '\0';
}

void obj_get_filepath_with_prefix(char * dest, char const * prefix)
{
	size_t len = strnlen(prefix, OBJ_FILENAME_LENGTH);
	strncpy(dest, prefix, len);
	strncpy(dest+len, strtok(NULL, NEWLINE), OBJ_FILENAME_LENGTH-len);
}
...
void obj_parse_mtl_line(char * current_line, list *material_list, obj_material ** current_mtl_ptr, char* material_open, char const * path_prefix, int line_number)
{
	char * current_token = strtok( current_line, " \t\n\r");
	obj_material * current_mtl = *current_mtl_ptr;

	//skip comments
	if( current_token == NULL || *current_token == '\0' || strequal(current_token, "//") || strequal(current_token, "#"))
		return;
	...
	{
		*material_open = 1;
		*current_mtl_ptr = (obj_material*) malloc(sizeof(obj_material));
		current_mtl = *current_mtl_ptr;
	...
	{
		current_mtl->transmission[0] = atof( strtok(NULL, " \t"));
		current_mtl->transmission[1] = atof( strtok(NULL, " \t"));
		current_mtl->transmission[2] = atof( strtok(NULL, " \t"));
	}
	//shiny
	...
	{
		current_mtl->illumination = atoi( strtok(NULL, " \t"));
	}
	// texture maps
	else if( strequal(current_token, "map_Ka") && *material_open)
	{
		obj_get_filepath_with_prefix(current_mtl->ambient_texture_filename, path_prefix);
	}
	else if( strequal(current_token, "map_Kd") && *material_open)
	{
		obj_get_filepath_with_prefix(current_mtl->diffuse_texture_filename, path_prefix);
	}
	else if( strequal(current_token, "map_Ks") && *material_open)
	{
		obj_get_filepath_with_prefix(current_mtl->specular_texture_filename, path_prefix);
	}
	else if( strequal(current_token, "map_Ns") && *material_open)
	{
		obj_get_filepath_with_prefix(current_mtl->spec_exp_texture_filename, path_prefix);
	}
	else if( strequal(current_token, "map_d") && *material_open)
	{
		obj_get_filepath_with_prefix(current_mtl->dissolve_texture_filename, path_prefix);
	}
	else if( strequal(current_token, "decal") && *material_open)
	{
		obj_get_filepath_with_prefix(current_mtl->decal_texture_filename, path_prefix);
	}
	else if( strequal(current_token, "disp") && *material_open)
	{
		obj_get_filepath_with_prefix(current_mtl->displacement_texture_filename, path_prefix);
	}
	else if( strequal(current_token, "bump") && *material_open)
	{
		obj_get_filepath_with_prefix(current_mtl->bump_texture_filename, path_prefix);
	}
	else if( strequal(current_token, "refl") && *material_open)
	{
		obj_get_filepath_with_prefix(current_mtl->reflect_texture_filename, path_prefix);
	}
	else
	{
		fprintf(stderr, "Unknown command '%s' in material file at line %i\n",
				current_token, line_number);
		//return 0;
	}
}

int obj_parse_mtl_file(char *filename, list *material_list, char const * path_prefix)
{
	int line_number = 0;
	char *current_token;
	...
	{
		line_number++;
		obj_parse_mtl_line(current_line, material_list, &current_mtl, &material_open, path_prefix, line_number);
	}
...
int obj_parse_mtl_string(char const * mtl_string, list *material_list, char const * path_prefix)
{
	int line_number = 0;
	const char *current_line = mtl_string;
	char parse_line[OBJ_LINE_SIZE];
	char material_open = 0;
	obj_material *current_mtl = NULL;

	list_make(material_list, 10, 1);

	current_line = get_next_line(parse_line, current_line, OBJ_LINE_SIZE);

	while( *current_line != '\0' )
	{
		line_number++;
		obj_parse_mtl_line(parse_line, material_list, &current_mtl, &material_open, path_prefix, line_number);
		current_line = get_next_line(parse_line, current_line, OBJ_LINE_SIZE);
	}
...
void obj_parse_line(char * current_line, int * current_material, int line_number, obj_growable_scene_data *growable_data, char const * path_prefix, char enable_mtl_parse)
{
	char * current_token;
	...
	else if( strequal(current_token, "mtllib") ) // mtllib
	{
		if( !enable_mtl_parse )
			return;
		obj_get_filepath_with_prefix(growable_data->material_filename, path_prefix);
		obj_parse_mtl_file(growable_data->material_filename, &growable_data->material_list, path_prefix);
		return;
	}
...
int obj_parse_obj_file(obj_growable_scene_data *growable_data, char const *filename)
{
	...
	char const * last_slash = filename;
	while( NULL != strstr(last_slash, "/"))
		last_slash = strstr(last_slash, "/")+1;
	size_t slash_pos = last_slash - filename;
	char* path_prefix = (char*) malloc(sizeof(char)*(slash_pos+1));
	strncpy(path_prefix, filename, slash_pos);
	path_prefix[slash_pos] = '\0';
/*		
	...
	{
		line_number++;
		obj_parse_line(current_line, &current_material, line_number, growable_data, path_prefix, 1);
	}
...
int obj_parse_obj_string(obj_growable_scene_data *growable_data, char const * obj_data_string, char const * mtl_data_string)
{
	int current_material = -1;
	const char *rest_of_line = obj_data_string;
	char parse_line[OBJ_LINE_SIZE];
	int line_number = 0;

	rest_of_line = get_next_line(parse_line, rest_of_line, OBJ_LINE_SIZE);

	while( *parse_line != '\0' || *rest_of_line != '\0')
	{
		line_number++;
		obj_parse_line(parse_line, &current_material, line_number, growable_data, "", 0);
		rest_of_line = get_next_line(parse_line, rest_of_line, OBJ_LINE_SIZE);
	}
...
int parse_obj_scene_string(obj_scene_data *data_out, char const * obj_data_string, char const * mtl_data_string)
{
	...
	if( obj_parse_mtl_string(mtl_data_string, &growable_data.material_list, "") == 0)
		return 0;
	if( obj_parse_obj_string(&growable_data, obj_data_string, mtl_data_string) == 0)
		return 0;
